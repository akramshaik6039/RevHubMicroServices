...
export class PostService {
  ...
  getPosts(page: number = 0, size: number = 10, feedType: string = 'universal', followingIds?: number[]): Observable<PageResponse<Post>> {
    ...
    if (followingIds && followingIds.length > 0) {
      followingIds.forEach(id => {
        params = params.append('followingIds', id.toString());
      });
    }
  ...
  addReply(commentId: number, content: string): Observable<any> {
    return this.http.post(`/posts/comments/${commentId}/replies`, { content });
  }
  
  searchPosts(query: string): Observable<Post[]> {
    return this.http.get<Post[]>(`${this.apiUrl}/search?q=${encodeURIComponent(query)}`);
  }
}
