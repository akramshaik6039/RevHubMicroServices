...
export class AuthService {
  ...
  getFollowers(): Observable<any[]> {
    return this.http.get<any[]>('/api/users/followers');
  }

  searchFollowers(query: string): Observable<any[]> {
    return this.http.get<any[]>(`/api/users/followers/search?q=${query || ''}`);
  }
  
  verifyOTP(email: string, otp: string): Observable<JwtResponse> {
    return this.http.post<JwtResponse>(`${this.apiUrl}/verify-otp`, { email, otp });
  }
  ...
  getHashtagSuggestions(query?: string): Observable<string[]> {
    return this.http.get<string[]>(`/search/hashtags?q=${query || ''}`);
  }
}